"""Phantom Millennium Python backend. Must set sys.path before any local imports."""

import os
import sys

# Millennium may set cwd elsewhere; local imports only resolve if backend/ is on sys.path.
def _bootstrap_backend_sys_path() -> None:
    try:
...
def _copy_webkit_files() -> None:
    ...
    logger.log(f"Copying Phantom web UI from {js_src} to {js_dst}")
    try:
        shutil.copy(js_src, js_dst)
    except Exception as exc:
        logger.error(f"Failed to copy Phantom web UI: {exc}")
    ...
    if os.path.exists(icon_src):
        try:
            ...
            logger.log(f"Copied Phantom icon to {icon_dst}")
        except Exception as exc:
            logger.error(f"Failed to copy Phantom icon: {exc}")
    else:
        logger.warn(f"Phantom icon not found at {icon_src}")
...
def _inject_webkit_files() -> None:
    """Register phantom.js with Millennium; must not raise — failures are logged only."""
    try:
        js_path = os.path.join(WEBKIT_DIR_NAME, WEB_UI_JS_FILE)
        Millennium.add_browser_js(js_path)
        logger.log(f"Phantom injected web UI: {js_path}")
    except Exception as exc:
        logger.error(f"Phantom: add_browser_js failed (UI may be limited): {exc}")
...
def HasPhantomForApp(appid: int, contentScriptQuery: str = "") -> str:
    return has_phantom_for_app(appid)


def StartAddViaPhantom(appid: int, contentScriptQuery: str = "") -> str:
    return start_add_via_phantom(appid)


def GetAddViaPhantomStatus(appid: int, contentScriptQuery: str = "") -> str:
...
def CancelAddViaPhantom(appid: int, contentScriptQuery: str = "") -> str:
    return cancel_add_via_phantom(appid)
...
def DeletePhantomForApp(appid: int, contentScriptQuery: str = "") -> str:
    return delete_phantom_for_app(appid)
...
def OpenExternalUrl(url: str, contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"Phantom: OpenExternalUrl failed: {exc}")
...
def GetSettingsConfig(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"Phantom: GetSettingsConfig failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def ApplySettingsChanges(
    ...
    try:
        ...
        try:
            logger.log(
                "Phantom: ApplySettingsChanges raw argument "
                f"type={type(changes)} value={changes!r}"
            )
            logger.log(f"Phantom: ApplySettingsChanges kwargs: {kwargs}")
        except Exception:
            pass
        ...
        if isinstance(changes, str) and changes:
            try:
                ...
                logger.warn("Phantom: Failed to parse changes string payload")
                ...
                if isinstance(payload, dict) and "changes" in payload:
                    ...
                    try:
                        ...
                        logger.warn("Phantom: Failed to parse changesJson string inside payload")
                        return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if "changesJson" in changes and isinstance(changes["changesJson"], str):
                try:
                    ...
                    logger.warn("Phantom: Failed to parse changesJson payload from dict")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if isinstance(changes_json, dict):
                ...
                try:
                    ...
                    logger.warn("Phantom: Failed to parse changesJson payload")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            else:
                payload = changes
        ...
        if payload is None:
            ...
            logger.warn(f"Phantom: Parsed payload is not a dict: {payload!r}")
            return json.dumps({"success": False, "error": "Invalid payload format"})

        try:
            logger.log(f"Phantom: ApplySettingsChanges received payload: {payload}")
        except Exception:
            pass

        result = apply_settings_changes(payload)
        try:
            logger.log(f"Phantom: ApplySettingsChanges result: {result}")
        except Exception:
            pass
        response = json.dumps(result)
        try:
            logger.log(f"Phantom: ApplySettingsChanges response json: {response}")
        except Exception:
            pass
        return response
    except Exception as exc:
        logger.warn(f"Phantom: ApplySettingsChanges failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableLocales(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"Phantom: GetAvailableLocales failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetTranslations(contentScriptQuery: str = "", language: str = "", **kwargs: Any) -> str:
    try:
        ...
        logger.warn(f"Phantom: GetTranslations failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableThemes(contentScriptQuery: str = "") -> str:
    ...
    try:
        ...
        logger.warn(f"Phantom: GetAvailableThemes failed: {exc}")
...
class Plugin:
    ...
    def _load(self):
        # Millennium requires ready() to run even if init steps fail; otherwise the
        # backend exits with code 1 ("plugin stopped unexpectedly").
        try:
            try:
                ver = Millennium.version()
            except Exception:
                ver = "unknown"
            logger.log(f"bootstrapping Phantom plugin, millennium {ver}")

            try:
                detect_steam_install_path()
            except Exception as exc:
                logger.warn(f"Phantom: steam path detection failed: {exc}")

            try:
                ensure_http_client("InitApis")
            except Exception as exc:
                logger.error(f"Phantom: HTTP client init failed: {exc}")
            ...
            try:
                ...
                logger.warn(f"Phantom: settings initialization failed: {exc}")
            ...
            # Never block plugin startup on heavy applist init/download.
            # Millennium may treat slow startup as a crash (exit code 1).
            def _init_applist_bg() -> None:
                try:
                    init_applist()
                except Exception as exc:
                    logger.warn(f"Phantom: Applist background initialization failed: {exc}")

            try:
                threading.Thread(target=_init_applist_bg, daemon=True).start()
            except Exception as exc:
                logger.warn(f"Phantom: Failed to start applist background thread: {exc}")

            try:
                _copy_webkit_files()
            except Exception as exc:
                logger.error(f"Phantom: _copy_webkit_files failed: {exc}")
            ...
            logger.error(f"Phantom: _load unexpected error: {exc}")
        finally:
            try:
                Millennium.ready()
            except Exception as exc:
                logger.error(f"Phantom: Millennium.ready() failed: {exc}")
