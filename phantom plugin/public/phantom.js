// Phantom button injection (standalone plugin)
...
(function () {
    ...
    gamepadCSS.textContent = `
        ...
        .phantom-button.active-focus,
        .phantom-restart-button.active-focus,
        .phantom-icon-button.active-focus {
    ...
    // ALL Phantom overlays that should block Steam navigation
    const OVERLAY_SELECTORS = [
        '.phantom-overlay',
        '.phantom-settings-overlay',
        '.phantom-fixes-results-overlay',
        '.phantom-loading-fixes-overlay',
        '.phantom-unfix-overlay',
        '.phantom-settings-manager-overlay',
        '.phantom-alert-overlay',
        '.phantom-confirm-overlay',
        '.phantom-loadedapps-overlay'
    ];
    ...
    function scanFocusableElements() {
        ...
        // Only scan if there's a Phantom overlay active
        const activeOverlay = document.querySelector(OVERLAY_SELECTOR_STRING);

        if (!activeOverlay) {
            console.log('[Gamepad] No Phantom overlay active, skipping scan');
    ...
    function pollGamepad() {
        ...
        // Check if there's an active Phantom overlay
...
(function () {
    ...
    function backendLog(message) {
        try {
            if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                Millennium.callServerMethod('phantom', 'Logger.log', {
                    message: String(message)
                });
            }
        } catch (err) {
            if (typeof console !== 'undefined' && console.warn) {
                console.warn('[Phantom] backendLog failed', err);
            }
        }
    }

    backendLog('Phantom script loaded');
    ...
    function fetchGamesDatabase() {
        ...
        return Millennium.callServerMethod('phantom', 'GetGamesDatabase', {
            contentScriptQuery: ''
        })
            .then(function (res) {
            ...
            .catch(function (err) {
                console.warn('[Phantom] Failed to fetch games database', err);
                return {};
            });
    }
    ...
    function fetchFixes(appid) {
        ...
        return Millennium.callServerMethod('phantom', 'CheckForFixes', {
            appid: appid,
            contentScriptQuery: ''
        })
            .then(function (res) {
            ...
            .catch(function (err) {
                console.warn('[Phantom] Failed to fetch fixes', err);
                return null;
            });
    }
    ...
    function fetchSteamGameName(appid) {
        ...
        return fetch('https://store.steampowered.com/api/appdetails?appids=' + appid + '&filters=basic')
            ...
            .catch(function (err) {
                backendLog('Phantom: fetchSteamGameName error for ' + appid + ': ' + err);
                return null;
            });
    }
    ...
    function applyTranslationBundle(bundle) {
        ...
        const stored = window.__PhantomI18n || {};
        if (bundle.language) {
    ...
    function loadThemesFromBackend() {
        ...
        return Millennium.callServerMethod('phantom', 'GetThemes', {
            contentScriptQuery: ''
        }).then(function (res) {
            try {
                const payload = typeof res === 'string' ? JSON.parse(res) : res;
    ...
    function getCurrentThemeKey() {
        try {
            const settings = window.__PhantomSettings || {};
    ...
    function getCurrentTheme() {
        try {
            ...
            if (!THEMES[themeName]) {
                try {
                    backendLog('Phantom: Theme ' + themeName + ' not found in THEMES, using original. Available: ' + Object.keys(THEMES).join(', '));
                } catch (_) { }
            }
            return theme;
        } catch (e) {
    ...
    function generateThemeStyles(theme) {
        return `
            ...
            .phantom-settings-overlay select,
            .phantom-settings-manager-overlay select,
            .phantom-overlay select,
            .phantom-fixes-results-overlay select,
            .phantom-loadedapps-overlay select {
            ...
            .phantom-settings-overlay select option,
            .phantom-settings-manager-overlay select option,
            .phantom-overlay select option,
            .phantom-fixes-results-overlay select option,
            .phantom-loadedapps-overlay select option {
                background-color: ${theme.bgPrimary} !important;
                color: ${theme.text} !important;
            }
            .phantom-settings-overlay select option:checked,
            .phantom-settings-manager-overlay select option:checked,
            .phantom-overlay select option:checked,
            .phantom-fixes-results-overlay select option:checked,
            .phantom-loadedapps-overlay select option:checked {
                background: ${theme.accent} !important;
                color: ${theme.text} !important;
            }
            .phantom-settings-overlay select:hover,
            .phantom-settings-manager-overlay select:hover,
            .phantom-overlay select:hover,
            .phantom-fixes-results-overlay select:hover,
            .phantom-loadedapps-overlay select:hover {
                border-color: ${theme.borderHover} !important;
            }
            .phantom-settings-overlay select:focus,
            .phantom-settings-manager-overlay select:focus,
            .phantom-overlay select:focus,
            .phantom-fixes-results-overlay select:focus,
            .phantom-loadedapps-overlay select:focus {
            ...
            .phantom-btn:hover:not([data-disabled="1"]) {
            ...
            .phantom-btn.primary:hover:not([data-disabled="1"]) {
    ...
    function ensureThemeStylesheet(themeKey) {
        ...
        try {
            ...
            backendLog('Phantom: Theme CSS injection failed: ' + err);
        }
    }

    function ensurePhantomStyles() {
        const styleEl = document.getElementById('phantom-styles');
        ...
        if (styleEl) {
            ...
            try {
                ...
                backendLog('Phantom: Styles injection failed: ' + err);
            }
        }
    }

    function ensureFontAwesome() {
        if (document.getElementById('phantom-fontawesome')) return;
        try {
            const link = document.createElement('link');
            link.id = 'phantom-fontawesome';
            link.rel = 'stylesheet';
            ...
            backendLog('Phantom: Font Awesome injection failed: ' + err);
        }
    }

    function showSettingsPopup() {
        if (document.querySelector('.phantom-settings-overlay') || settingsMenuPending) return;
        settingsMenuPending = true;
        ensureTranslationsLoaded(false).catch(function () {
            return null;
        }).finally(function () {
            settingsMenuPending = false;
            if (document.querySelector('.phantom-settings-overlay')) return;

            try {
                const d = document.querySelector('.phantom-overlay');
                if (d) d.remove();
            } catch (_) { }
            ensurePhantomStyles();
            ensureFontAwesome();
            ...
            overlay.className = 'phantom-settings-overlay';
            ...
            title.textContent = t('menu.title', 'Phantom · Menu');
            ...
            const discordBtn = createIconButton('phantom-settings-discord', 'fa-brands fa-discord', 'menu.discord', 'Discord');
            const settingsManagerBtn = createIconButton('phantom-settings-open-manager', 'fa-gear', 'menu.settings', 'Settings');
            const closeBtn = createIconButton('phantom-settings-close', 'fa-xmark', 'settings.close', 'Close');

            createSectionLabel('menu.manageGameLabel', 'Manage Game');

            const removeBtn = createMenuButton('phantom-settings-remove-game', 'menu.removePhantom', 'Remove via Phantom', 'fa-trash-can');
            removeBtn.style.display = 'none';

            const fixesMenuBtn = createMenuButton('phantom-settings-fixes-menu', 'menu.fixesMenu', 'Fixes Menu', 'fa-wrench');

            createSectionLabel('menu.advancedLabel', 'Advanced');
            const checkBtn = createMenuButton('phantom-settings-check', 'menu.checkForUpdates', 'Check For Updates', 'fa-cloud-arrow-down');
            const fetchApisBtn = createMenuButton('phantom-settings-fetch-apis', 'menu.fetchFreeApis', 'Fetch Free APIs', 'fa-server');
            ...
            if (checkBtn) {
                checkBtn.addEventListener('click', function (e) {
                    ...
                    try {
                        Millennium.callServerMethod('phantom', 'CheckForUpdatesNow', {
                            contentScriptQuery: ''
                        }).then(function (res) {
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                const msg = (payload && payload.message) ? String(payload.message) : lt('No updates available.');
                                ShowPhantomAlert('Phantom', msg);
                            } catch (_) { }
                        });
                    } catch (_) { }
                });
            ...
            if (discordBtn) {
                discordBtn.addEventListener('click', function (e) {
                    ...
                    const url = 'https://discord.gg/phantom';
                    try {
                        Millennium.callServerMethod('phantom', 'OpenExternalUrl', {
                            url,
                            contentScriptQuery: ''
                        });
                    } catch (_) { }
            ...
            if (fetchApisBtn) {
                fetchApisBtn.addEventListener('click', function (e) {
                    ...
                    try {
                        Millennium.callServerMethod('phantom', 'FetchFreeApisNow', {
                            contentScriptQuery: ''
                        }).then(function (res) {
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
            ...
            if (fixesMenuBtn) {
                fixesMenuBtn.addEventListener('click', function (e) {
                    ...
                    try {
                        ...
                        const appid = match ? parseInt(match[1], 10) : (window.__PhantomCurrentAppId || NaN);
                        if (isNaN(appid)) {
                            try {
                                overlay.remove();
                            } catch (_) { }
                            const errText = t('menu.error.noAppId', 'Could not determine game AppID');
                            ShowPhantomAlert('Phantom', errText);
                            return;
                        }

                        Millennium.callServerMethod('phantom', 'GetGameInstallPath', {
                            appid,
                            contentScriptQuery: ''
                            ...
                            try {
                                ...
                                if (pathPayload && pathPayload.success && pathPayload.installPath) {
                                    ...
                                    window.__PhantomGameInstallPath = pathPayload.installPath;
                                }
                                window.__PhantomGameIsInstalled = isGameInstalled;
                                try {
                                    overlay.remove();
                                } catch (_) { }
                                showFixesLoadingPopupAndCheck(appid);
                            } catch (err) {
                                backendLog('Phantom: GetGameInstallPath error: ' + err);
                                try {
                                    overlay.remove();
                                } catch (_) { }
                            }
                            ...
                            ShowPhantomAlert('Phantom', errorText);
                        });
                    } catch (err) {
                        backendLog('Phantom: Fixes Menu button error: ' + err);
                    }
                });
            }
            ...
            try {
                ...
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    Millennium.callServerMethod('phantom', 'HasPhantomForApp', {
                        appid,
                        contentScriptQuery: ''
                    }).then(function (res) {
                        try {
                            ...
                            if (exists) {
                                const doDelete = function () {
                                    try {
                                        Millennium.callServerMethod('phantom', 'DeletePhantomForApp', {
                                            appid,
                                            contentScriptQuery: ''
                                        }).then(function () {
                                            try {
                                                window.__PhantomButtonInserted = false;
                                                window.__PhantomPresenceCheckInFlight = false;
                                                window.__PhantomPresenceCheckAppId = undefined;
                                                addPhantomButton();
                                                const successText = t('menu.remove.success', 'Phantom removed for this app.');
                                                ShowPhantomAlert('Phantom', successText);
                                            } catch (err) {
                                                backendLog('Phantom: post-delete cleanup failed: ' + err);
                                            }
                                        }).catch(function (err) {
                                            const failureText = t('menu.remove.failure', 'Failed to remove Phantom.');
                                            const errMsg = (err && err.message) ? err.message : failureText;
                                            ShowPhantomAlert('Phantom', errMsg);
                                        });
                                    } catch (err) {
                                        backendLog('Phantom: doDelete failed: ' + err);
                                    }
                                };
                                ...
                                removeBtn.onclick = function (e) {
                                    ...
                                    const confirmMessage = t('menu.remove.confirm', 'Remove via Phantom for this game?');
                                    showPhantomConfirm('Phantom', confirmMessage, function () {
                                        doDelete();
                                    }, function () {
                                        try {
                                            showSettingsPopup();
    ...
    function ensureTranslationsLoaded(forceRefresh, preferredLanguage) {
        try {
            if (!forceRefresh && window.__PhantomI18n && window.__PhantomI18n.ready) {
                return Promise.resolve(window.__PhantomI18n);
            }
            if (typeof Millennium === 'undefined' || typeof Millennium.callServerMethod !== 'function') {
                window.__PhantomI18n = window.__PhantomI18n || {
                    language: 'en',
                    locales: [],
                    strings: {},
                    ready: false
            ...
            const settingsVals = ((window.__PhantomSettings || {}).values || {}).general || {};
            ...
            if (!targetLanguage) {
                ...
                targetLanguage = useSteamLang ? steamLang : ((window.__PhantomI18n && window.__PhantomI18n.language) || 'en');
            }
            return Millennium.callServerMethod('phantom', 'GetTranslations', {
                ...
                return window.__PhantomI18n;
            }).catch(function (err) {
                backendLog('Phantom: translation load failed: ' + err);
                window.__PhantomI18n = window.__PhantomI18n || {
                    language: 'en',
                    locales: [],
                    strings: {},
            ...
            backendLog('Phantom: ensureTranslationsLoaded error: ' + err);
            window.__PhantomI18n = window.__PhantomI18n || {
                language: 'en',
                locales: [],
                strings: {},
    ...
    function translateText(key, fallback) {
        ...
        try {
            const store = window.__PhantomI18n;
    ...
    function showTestPopup() {
        ...
        // Avoid duplicates
        if (document.querySelector('.phantom-overlay')) return;
        // Close settings popup if open so modals don't overlap
        try {
            const s = document.querySelector('.phantom-settings-overlay');
            if (s) s.remove();
        } catch (_) { }
        ...
        title.className = 'phantom-title';
        title.textContent = t('common.appName', 'Phantom');

        // API list container
        const apiListContainer = document.createElement('div');
        apiListContainer.className = 'phantom-api-list';
        ...
        if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
            Millennium.callServerMethod('phantom', 'GetApiList', {
                contentScriptQuery: ''
            }).then(function (res) {
                try {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success && payload.apis && Array.isArray(payload.apis)) {
                        ...
                        payload.apis.forEach((api, index) => {
                            ...
                            apiItem.className = `phantom-api-item phantom-api-${index}`;
        ...
        progressWrap.className = 'phantom-progress-wrap';
        ...
        cancelBtn.className = 'phantom-btn phantom-cancel-btn';
        ...
        hideBtn.className = 'phantom-btn phantom-hide-btn';
        ...
        function cancelOperation() {
            ...
            try {
                ...
                const appid = match ? parseInt(match[1], 10) : (window.__PhantomCurrentAppId || NaN);
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    Millennium.callServerMethod('phantom', 'CancelAddViaPhantom', {
                        appid,
                        contentScriptQuery: ''
                    });
                }
            ...
            const status = overlay.querySelector('.phantom-status');
            if (status) status.textContent = lt('Cancelled');
            const cancelBtn = overlay.querySelector('.phantom-cancel-btn');
            if (cancelBtn) cancelBtn.style.display = 'none';
            const hideBtn = overlay.querySelector('.phantom-hide-btn');
            if (hideBtn) hideBtn.innerHTML = `<span>${lt('Close')}</span>`;
            // Hide progress UI
            const wrap = overlay.querySelector('.phantom-progress-wrap');
            const progressInfo = overlay.querySelector('.phantom-progress-info');
    ...
    function showFixesResultsPopup(data, isGameInstalled) {
        if (document.querySelector('.phantom-fixes-results-overlay')) return;
        // Close other popups
        try {
            const d = document.querySelector('.phantom-overlay');
            if (d) d.remove();
        ...
        try {
            const f = document.querySelector('.phantom-fixes-results-overlay');
            if (f) f.remove();
        } catch (_) { }
        try {
            const l = document.querySelector('.phantom-loading-fixes-overlay');
            if (l) l.remove();
        } catch (_) { }
        ...
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'phantom-fixes-results-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        ...
        const title = document.createElement('div');
        title.style.cssText = `font-size:24px;color:${colors.text};font-weight:700;text-shadow:0 2px 8px ${colors.shadow};background:${colors.gradientLight};-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;`;
        title.textContent = lt('Phantom · Fixes Menu');
        ...
        function createFixButton(label, text, icon, isSuccess, onClick) {
            ...
            try {
                const currentThemeKey = (((window.__PhantomSettings || {}).values || {}).general || {}).theme || 'original';
        ...
        const unfixSection = createFixButton(
            ...
            function (e) {
                ...
                if (isGameInstalled) {
                    ...
                    showPhantomConfirm('Phantom', lt('Are you sure you want to un-fix? This will remove fix files and verify game files.'),
                        function () {
        ...
        gameFolderBtn.className = 'phantom-btn';
        ...
        gameFolderBtn.onclick = function (e) {
            e.preventDefault();
            if (window.__PhantomGameInstallPath) {
                try {
                    Millennium.callServerMethod('phantom', 'OpenGameFolder', {
                        path: window.__PhantomGameInstallPath,
                        contentScriptQuery: ''
                    });
                } catch (err) {
                    backendLog('Phantom: Failed to open game folder: ' + err);
                }
            }
        };
        ...
        function startUnfix(appid) {
            try {
                Millennium.callServerMethod('phantom', 'UnFixGame', {
                    appid: appid,
                    installPath: window.__PhantomGameInstallPath,
                    contentScriptQuery: ''
                }).then(function (res) {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success) {
                        ...
                        ShowPhantomAlert('Phantom', errorMsg);
                    }
                }).catch(function () {
                ...
                backendLog('Phantom: Un-Fix start error: ' + err);
            }
        }
    }

    function showFixesLoadingPopupAndCheck(appid) {
        if (document.querySelector('.phantom-loading-fixes-overlay')) return;
        try {
            const d = document.querySelector('.phantom-overlay');
            if (d) d.remove();
        } catch (_) { }
        ...
        try {
            const f = document.querySelector('.phantom-fixes-overlay');
            if (f) f.remove();
        } catch (_) { }
        ...
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'phantom-loading-fixes-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        ...
        fetchFixes(appid).then(function (payload) {
            if (payload && payload.success) {
                const isGameInstalled = window.__PhantomGameIsInstalled === true;
    ...
    function applyFix(appid, downloadUrl, fixType, gameName, resultsOverlay) {
        try {
            ...
            backendLog('Phantom: Applying fix ' + fixType + ' for appid ' + appid);

            // Start the download and extraction process
            Millennium.callServerMethod('phantom', 'ApplyGameFix', {
                appid: appid,
                ...
                try {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success) {
                        // Show download progress popup similar to Add via Phantom
                    ...
                    backendLog('Phantom: ApplyGameFix response error: ' + err);
                    const msg = lt('Error applying fix');
                    ShowPhantomAlert('Phantom', msg);
                }
            }).catch(function (err) {
                backendLog('Phantom: ApplyGameFix error: ' + err);
                const msg = lt('Error applying fix');
                ShowPhantomAlert('Phantom', msg);
            });
        } catch (err) {
            backendLog('Phantom: applyFix error: ' + err);
        }
    }

    // Show fix download progress popup
    function showFixDownloadProgress(appid, fixType) {
        // Reuse the download popup UI from Add via Phantom
        if (document.querySelector('.phantom-overlay')) return;
        ...
        const cancelBtn = document.createElement('a');
        cancelBtn.href = '#';
        cancelBtn.className = 'phantom-btn primary';
        ...
        cancelBtn.onclick = function (e) {
            ...
            Millennium.callServerMethod('phantom', 'CancelApplyFix', {
                appid: appid,
                contentScriptQuery: ''
            }).then(function (res) {
                try {
                    ...
                    backendLog('Phantom: CancelApplyFix response error: ' + err);
                ...
                const msgEl2 = document.getElementById('lt-fix-progress-msg');
                if (msgEl2 && msgEl2.dataset.last) msgEl2.textContent = msgEl2.dataset.last;
                backendLog('Phantom: CancelApplyFix error: ' + err);
                const msg = lt('Failed to cancel fix download');
                ShowPhantomAlert('Phantom', msg);
            });
        };
    ...
    function replaceFixButtonsWithClose(overlayEl) {
        ...
        closeBtn.href = '#';
        closeBtn.className = 'phantom-btn primary';
    ...
    function pollFixProgress(appid, fixType) {
        const poll = function () {
            try {
                const overlayEl = document.querySelector('.phantom-overlay');
                if (!overlayEl) return; // Stop if overlay was closed

                Millennium.callServerMethod('phantom', 'GetApplyFixStatus', {
                    appid: appid,
                    contentScriptQuery: ''
                }).then(function (res) {
                    try {
                        ...
                        backendLog('Phantom: GetApplyFixStatus error: ' + err);
                    }
                });
            } catch (err) {
                backendLog('Phantom: pollFixProgress error: ' + err);
            }
        };
        setTimeout(poll, 500);
    }
    ...
    function showUnfixProgress(appid) {
        ...
        try {
            const old = document.querySelector('.phantom-unfix-overlay');
            if (old) old.remove();
        } catch (_) { }
        ...
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'phantom-unfix-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
    ...
    function pollUnfixProgress(appid) {
        const poll = function () {
            try {
                const overlayEl = document.querySelector('.phantom-unfix-overlay');
                if (!overlayEl) return; // Stop if overlay was closed

                Millennium.callServerMethod('phantom', 'GetUnfixStatus', {
                    appid: appid,
                    contentScriptQuery: ''
                }).then(function (res) {
                    try {
                            ...
                            if (state.status === 'removing') {
                                ...
                                setTimeout(function () {
                                    try {
                                        ...
                                        backendLog('Phantom: Running verify for appid ' + appid);
                                    } catch (_) { }
                                }, 1000);
                        ...
                        backendLog('Phantom: GetUnfixStatus error: ' + err);
                    }
                });
            } catch (err) {
                backendLog('Phantom: pollUnfixProgress error: ' + err);
            }
        };
        setTimeout(poll, 500);
    }
    ...
    function fetchSettingsConfig(forceRefresh) {
        try {
            if (!forceRefresh && window.__PhantomSettings && Array.isArray(window.__PhantomSettings.schema)) {
                return Promise.resolve(window.__PhantomSettings);
            }
        } catch (_) { }

        if (typeof Millennium === 'undefined' || typeof Millennium.callServerMethod !== 'function') {
            return Promise.reject(new Error(lt('Phantom backend unavailable')));
        }

        return Millennium.callServerMethod('phantom', 'GetSettingsConfig', {
            contentScriptQuery: ''
        }).then(function (res) {
            const payload = typeof res === 'string' ? JSON.parse(res) : res;
            if (!payload || payload.success !== true) {
            ...
            window.__PhantomSettings = config;
            return config;
        });
    }
    ...
    function showSettingsManagerPopup(forceRefresh, onBack) {
        if (document.querySelector('.phantom-settings-manager-overlay')) return;

        try {
            const mainOverlay = document.querySelector('.phantom-settings-overlay');
        ...
        const overlay = document.createElement('div');
        overlay.className = 'phantom-settings-manager-overlay';
        ...
        title.textContent = t('settings.title', 'Phantom · Settings');
        ...
        searchInput.id = 'phantom-settings-search';
        ...
        contentWrap.id = 'phantom-content-wrap';
        ...
        function applySearchFilter() {
            ...
            const fixesSection = document.getElementById('phantom-installed-fixes-section');
            ...
            if (fixesSection && query && fixItems.length > 0 && visibleFixes === 0) {
                if (!fixesEmptySearch) {
                    ...
                    const listContainer = fixesSection.querySelector('#phantom-fixes-list');
            ...
            const luaSection = document.getElementById('phantom-installed-lua-section');
            ...
            if (luaSection && query && luaItems.length > 0 && visibleLua === 0) {
                if (!luaEmptySearch) {
                    ...
                    emptyEl.style.cssText = `padding:14px;background:${emptyColors.bgTertiary};border:1px solid ${emptyColors.border};border-radius:4px;color:${emptyColors.textSecondary};text-align:center;margin-top:10px;`;
                    emptyEl.textContent = t('settings.search.noResults', 'No matches found');
                    const listContainer = luaSection.querySelector('#phantom-lua-list');
                    if (listContainer) listContainer.appendChild(emptyEl);
                }
        ...
        function setStatus(text, color) {
            let statusLine = contentWrap.querySelector('.phantom-settings-status');
            if (!statusLine) {
                statusLine = document.createElement('div');
                statusLine.className = 'phantom-settings-status';
                ...
                for (let j = 0; j < options.length; j++) {
                    ...
                    if (option.type === 'select') {
                        ...
                        selectEl.addEventListener('change', function () {
                            ...
                            try {
                                backendLog('Phantom: ' + option.key + ' select changed to ' + selectEl.value);
                            } catch (_) { }

                            // If theme changed, apply it immediately
                            if (group.key === 'general' && option.key === 'theme') {
                                try {
                                    backendLog('Phantom: Theme change detected, new value: ' + selectEl.value);
                                } catch (_) { }
                                // Update the settings cache so getCurrentTheme() returns the new value
                                if (window.__PhantomSettings && window.__PhantomSettings.values) {
                                    if (!window.__PhantomSettings.values.general) {
                                        window.__PhantomSettings.values.general = {};
                                    }
                                    window.__PhantomSettings.values.general.theme = selectEl.value;
                                    try {
                                        backendLog('Phantom: Updated cache, theme is now: ' + window.__PhantomSettings.values.general.theme);
                                    } catch (_) { }
                                }
                                ...
                                setTimeout(function () {
                                    ...
                                    const contentWrapEl = overlay && overlay.querySelector('#phantom-content-wrap');
                                    if (contentWrapEl) {
        ...
        function renderInstalledFixesSection() {
            ...
            sectionEl.id = 'phantom-installed-fixes-section';
            ...
            listContainer.id = 'phantom-fixes-list';
        ...
        function loadInstalledFixes(container) {
            ...
            Millennium.callServerMethod('phantom', 'GetInstalledFixes', {
                contentScriptQuery: ''
            })
                .then(function (res) {
        ...
        function renderInstalledLuaSection() {
            const sectionEl = document.createElement('div');
            sectionEl.id = 'phantom-installed-lua-section';
            ...
            const listContainer = document.createElement('div');
            listContainer.id = 'phantom-lua-list';
            listContainer.style.cssText = 'min-height:50px;';
            sectionEl.appendChild(listContainer);
        ...
        function loadInstalledLuaScripts(container) {
            ...
            Millennium.callServerMethod('phantom', 'GetInstalledLuaScripts', {
                contentScriptQuery: ''
            })
                .then(function (res) {
                    const response = typeof res === 'string' ? JSON.parse(res) : res;
                    ...
                    if (hasUnknownGames) {
                        ...
                        infoBanner.innerHTML = '<i class="fa-solid fa-circle-info" style="font-size:16px;"></i><span>' + t('settings.installedLua.unknownInfo', 'Games showing \'Unknown Game\' were installed manually (not via Phantom).') + '</span>';
        ...
        function createLuaListItem(script, container) {
            ...
            deleteBtn.addEventListener('click', function (e) {
                ...
                showPhantomConfirm(
                    script.gameName || 'Phantom',
                    t('settings.installedLua.deleteConfirm', 'Remove via Phantom for this game?'),
                    function () {
                        // User confirmed
                        deleteBtn.dataset.busy = '1';
                        deleteBtn.style.opacity = '0.6';
        ...
        saveBtn.addEventListener('click', function (e) {
            ...
            try {
                backendLog('Phantom: collectChanges payload ' + JSON.stringify(changes));
            } catch (_) { }
            ...
            try {
                backendLog('Phantom: sending settings payload ' + JSON.stringify(payloadToSend));
            } catch (_) { }
            // Pass flattened keys so Millennium handles the RPC arguments as expected.
            Millennium.callServerMethod('phantom', 'ApplySettingsChanges', {
                contentScriptQuery: '',
                ...
                try {
                    if (window.__PhantomSettings) {
                        window.__PhantomSettings.values = JSON.parse(JSON.stringify(state.config.values));
                        window.__PhantomSettings.schemaVersion = state.config.schemaVersion;
                        window.__PhantomSettings.lastFetched = Date.now();
                        if (response && response.translations && typeof response.translations === 'object') {
                            window.__PhantomSettings.translations = response.translations;
                        }
                        if (response && response.language) {
                            window.__PhantomSettings.language = response.language;
                        }
                    }
                } catch (_) { }
                ...
                try {
                    if (window.__PhantomSettings) {
                        window.__PhantomSettings.schema = null;
                    }
                } catch (_) { }

                if (response && response.translations && typeof response.translations === 'object') {
                    applyTranslationBundle({
                        language: response.language || (window.__PhantomI18n && window.__PhantomI18n.language) || 'en',
                        locales: (window.__PhantomI18n && window.__PhantomI18n.locales) || (state.config && state.config.locales) || [],
    ...
    function closeSettingsOverlay() {
        try {
            ...
            var list = document.getElementsByClassName('phantom-settings-overlay');
            ...
            var list2 = document.getElementsByClassName('phantom-overlay');
    ...
    function showPhantomAlert(title, message, onClose) {
        if (document.querySelector('.phantom-alert-overlay')) return;

        ensurePhantomStyles();
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'phantom-alert-overlay';
        ...
        titleEl.textContent = String(title || 'Phantom');
        ...
        okBtn.href = '#';
        okBtn.className = 'phantom-btn primary';
    ...
    function ShowPhantomAlert(title, message) {
        try {
            showPhantomAlert(title, message);
        } catch (err) {
            backendLog('Phantom: Alert error, falling back: ' + err);
            try {
                alert(String(title) + '\n\n' + String(message));
            } catch (_) { }
        }
    ...
    function showPhantomConfirm(title, message, onConfirm, onCancel) {
        // Always close settings popup first so the confirm is visible on top
        closeSettingsOverlay();

        // Create custom modern confirmation dialog
        if (document.querySelector('.phantom-confirm-overlay')) return;

        ensurePhantomStyles();
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'phantom-confirm-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(10px);z-index:100001;display:flex;align-items:center;justify-content:center;';
        ...
        const cancelBtn = document.createElement('a');
        cancelBtn.href = '#';
        cancelBtn.className = 'phantom-btn';
        cancelBtn.style.flex = '1';
        cancelBtn.innerHTML = `<span>${lt('Cancel')}</span>`;
        cancelBtn.onclick = function (e) {
            e.preventDefault();
        ...
        confirmBtn.href = '#';
        confirmBtn.className = 'phantom-btn primary';
    ...
    function showDlcWarning(appid, fullgameAppid, fullgameName) {
        ...
        closeSettingsOverlay();
        if (document.querySelector('.phantom-dlc-warning-overlay')) return;

        ensurePhantomStyles();
        ensureFontAwesome();

        const overlay = document.createElement('div');
        overlay.className = 'phantom-dlc-warning-overlay phantom-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(10px);z-index:100001;display:flex;align-items:center;justify-content:center;';
        ...
        goBtn.className = 'phantom-btn primary';
    ...
    function showPhantomPlayableWarning(message, onProceed, onCancel) {
        // Close settings so modal is visible
        closeSettingsOverlay();
        if (document.querySelector('.phantom-playable-warning-overlay')) return;

        ensurePhantomStyles();
        ensureFontAwesome();

        const overlay = document.createElement('div');
        overlay.className = 'phantom-playable-warning-overlay phantom-overlay';
        ...
        proceedBtn.href = '#';
        proceedBtn.className = 'phantom-btn primary';
    ...
    function showMillenniumDisclaimerModal() {
        if (document.querySelector('.phantom-disclaimer-overlay')) return;

        ensurePhantomStyles();
        ensureFontAwesome();

        const overlay = document.createElement('div');
        overlay.className = 'phantom-disclaimer-overlay phantom-overlay';
        ...
        line1.textContent = t('disclaimer.line1', 'Phantom is not affiliated in any way with Millennium');
        ...
        line3.textContent = t('disclaimer.line3', 'You will be BANNED from both Phantom and Millennium servers if you go to their discord asking for help');
        ...
        confirmBtn.onclick = function (e) {
            ...
            if (input.value.trim().toLowerCase() === expectedPhrase) {
                localStorage.setItem('phantom millennium disclaimer accepted', '1');
                overlay.remove();
            }
        };
    ...
    function ensureStyles() {
        if (!document.getElementById('phantom-spacing-styles')) {
            const style = document.createElement('style');
            style.id = 'phantom-spacing-styles';
            style.textContent = `
                .phantom-restart-button, .phantom-icon-button { margin-left: 6px !important; margin-right: 0 !important; }
                .phantom-button { margin-right: 0 !important; position: relative !important; }
                .phantom-pills-container {
                ...
                .phantom-pill.red { background: rgba(255, 80, 80, 0.15); color: #ff5050; border: 1px solid rgba(255, 80, 80, 0.3); }
                .phantom-pill.green { background: rgba(92, 184, 92, 0.15); color: #5cb85c; border: 1px solid rgba(92, 184, 92, 0.3); }
                .phantom-pill.yellow { background: rgba(255, 193, 7, 0.15); color: #ffc107; border: 1px solid rgba(255, 193, 7, 0.3); }
                .phantom-pill.orange { background: rgba(255, 136, 0, 0.15); color: #ff8800; border: 1px solid rgba(255, 136, 0, 0.3); }
                .phantom-pill.gray { background: rgba(150, 150, 150, 0.15); color: #a0a0a0; border: 1px solid rgba(150, 150, 150, 0.3); }
            `;
    ...
    function updateButtonTranslations() {
        try {
            ...
            const restartBtn = document.querySelector('.phantom-restart-button');
            if (restartBtn) {
            ...
            // Update Add via Phantom button
            const phantomBtn = document.querySelector('.phantom-button');
            if (phantomBtn) {
                const addViaText = lt('Add via Phantom');
                phantomBtn.title = addViaText;
                phantomBtn.setAttribute('data-tooltip-text', addViaText);
                const span = phantomBtn.querySelector('span');
                if (span) {
                    span.textContent = addViaText;
                }
            }
        } catch (err) {
            backendLog('Phantom: updateButtonTranslations error: ' + err);
        }
    }

    // Function to add the Phantom button
    ...
    function addPhantomButton() {
        ...
        if (window.__PhantomLastUrl !== currentUrl) {
            // Page changed - reset button insertion flag and update translations
            window.__PhantomLastUrl = currentUrl;
            window.__PhantomButtonInserted = false;
            window.__PhantomRestartInserted = false;
            window.__PhantomIconInserted = false;
            window.__PhantomHeaderInserted = false;
            window.__PhantomPresenceCheckInFlight = false;
            window.__PhantomPresenceCheckAppId = undefined;
        ...
        if (!isAppPath) {
            ...
            if (headerContainer && !document.querySelector('.phantom-header-button') && !window.__PhantomHeaderInserted) {
                ensurePhantomStyles();
                const headerBtn = document.createElement('a');
                headerBtn.href = '#';
                // Use phantom-btn primary class for that premium modal look
                headerBtn.className = 'phantom-btn primary phantom-header-button Focusable';
                ...
                headerBtn.title = 'Phantom Settings';

                headerBtn.setAttribute('data-tooltip-text', 'Phantom Settings');
                ...
                try {
                    Millennium.callServerMethod('phantom', 'GetIconDataUrl', {
                        contentScriptQuery: ''
                    }).then(function (res) {
                        try {
                            const payload = typeof res === 'string' ? JSON.parse(res) : res;
                            if (payload && payload.success && payload.dataUrl) {
                                ...
                                img.src = 'Phantom/phantom-icon.png';
                            }
                        } catch (_) {
                            img.src = 'Phantom/phantom-icon.png';
                        }
                ...
                window.__PhantomHeaderInserted = true;
        ...
        if (isBigPicture) {
            ...
            // Prefer native Steam UI; SteamDB extension is optional
            targetContainer = document.querySelector('.apphub_OtherSiteInfo') ||
                document.querySelector('.steamdb-buttons') ||
                document.querySelector('[data-steamdb-buttons]');
            if (!targetContainer) {
                const path = window.location.pathname || '';
                const isAppPage = /\/app\/\d+/.test(path);
                if (isAppPage) {
                    let row = document.querySelector('.phantom-store-button-row');
                    if (!row) {
                        const anchor = document.querySelector('.game_area_purchase_game_wrapper') ||
                            document.querySelector('.game_area_purchase_game') ||
                            document.querySelector('.glance_details');
                        if (anchor) {
                            row = document.createElement('div');
                            row.className = 'phantom-store-button-row';
                            row.style.cssText = 'margin-top:8px;display:flex;flex-wrap:wrap;gap:8px;align-items:center;width:100%;';
                            anchor.appendChild(row);
                        }
                    }
                    targetContainer = row || null;
                }
            }
        }
        ...
        if (targetContainer) {
            ...
            // Insert a Restart Steam button between Community Hub and our Phantom button
            try {
                if (!document.querySelector('.phantom-restart-button') && !window.__PhantomRestartInserted) {
                    ensureStyles();
                    ...
                    const referenceBtn = isBigPicture ?
                        ...
                        buttonHost.querySelector('a');

                    // Use same custom button for both modes
                    const restartBtn = document.createElement('a');
                    if (referenceBtn && referenceBtn.className) {
                        restartBtn.className = referenceBtn.className + ' phantom-restart-button';
                    } else {
                        restartBtn.className = 'btnv6_blue_hoverfade btn_medium phantom-restart-button';
                    }
                    restartBtn.href = '#';
                    const restartText = lt('Restart Steam');
                    restartBtn.title = restartText;
                    ...
                    restartBtn.addEventListener('click', function (e) {
                        ...
                        try {
                            ...
                            closeSettingsOverlay();
                            showPhantomConfirm('Phantom', lt('Restart Steam now?'),
                                function () {
                                    try {
                                        Millennium.callServerMethod('phantom', 'RestartSteam', {
                                            contentScriptQuery: ''
                                        });
                                    } catch (_) { }
                                },
                    ...
                    if (referenceBtn && referenceBtn.parentElement) {
                        ...
                        buttonHost.appendChild(restartBtn);
                    }
                    // Insert icon button right after Restart (only once)
                    try {
                        if (!document.querySelector('.phantom-icon-button') && !window.__PhantomIconInserted) {
                            // Use same custom button for both modes
                            const iconBtn = document.createElement('a');
                            if (referenceBtn && referenceBtn.className) {
                                iconBtn.className = referenceBtn.className + ' phantom-icon-button';
                            } else {
                                iconBtn.className = 'btnv6_blue_hoverfade btn_medium phantom-icon-button';
                            }
                            iconBtn.href = '#';
                            iconBtn.title = 'Phantom Helper';
                            iconBtn.setAttribute('data-tooltip-text', 'Phantom Helper');
                            ...
                            buttonHost.appendChild(iconBtn);

                            window.__PhantomIconInserted = true;
                            backendLog('Inserted Icon button');
                        }
                    } catch (_) { }
                    window.__PhantomRestartInserted = true;
            ...
            const existingBtn = document.querySelector('.phantom-button');
            if (existingBtn) {
                ensureTranslationsLoaded(false).then(function () {
                    updateButtonTranslations();
                });
            ...
            if (!existingBtn && !window.__PhantomButtonInserted) {

                // Create the Phantom button modeled after existing SteamDB/PCGW buttons
                // In Big Picture mode, use queue button as reference; otherwise use first link in container
                ...
                // Use same custom button for both modes
                const phantomButton = document.createElement('a');
                phantomButton.href = '#';
                // Copy classes from an existing button to match look-and-feel, but set our own label
                if (referenceBtn && referenceBtn.className) {
                    phantomButton.className = referenceBtn.className + ' phantom-button';
                } else {
                    phantomButton.className = 'btnv6_blue_hoverfade btn_medium phantom-button';
                }
                const span = document.createElement('span');
                const addViaText = lt('Add via Phantom');
                span.textContent = addViaText;
                phantomButton.appendChild(span);
                // Tooltip/title
                phantomButton.title = addViaText;
                phantomButton.setAttribute('data-tooltip-text', addViaText);

                // Normalize margins to match native buttons
                try {
                    if (referenceBtn) {
                        const cs = window.getComputedStyle(referenceBtn);
                        phantomButton.style.marginLeft = cs.marginLeft;
                        phantomButton.style.marginRight = cs.marginRight;
                    }
                } catch (_) { }

                // Local click handler suppressed; delegated handler manages actions
                phantomButton.addEventListener('click', function (e) {
                    e.preventDefault();
                    backendLog('Phantom button clicked (delegated handler will process)');
                });

                // Before inserting, ask backend if Phantom already exists for this appid
                try {
                    const match = window.location.href.match(/https:\/\/store\.steampowered\.com\/app\/(\d+)/) || window.location.href.match(/https:\/\/steamcommunity\.com\/app\/(\d+)/);
                    const appid = match ? parseInt(match[1], 10) : NaN;
                    if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                        // prevent multiple concurrent checks
                        if (window.__PhantomPresenceCheckInFlight && window.__PhantomPresenceCheckAppId === appid) {
                            return;
                        }
                        window.__PhantomPresenceCheckInFlight = true;
                        window.__PhantomPresenceCheckAppId = appid;
                        window.__PhantomCurrentAppId = appid;
                        Millennium.callServerMethod('phantom', 'HasPhantomForApp', {
                            appid,
                            contentScriptQuery: ''
                        }).then(function (res) {
                            try {
                                ...
                                if (payload && payload.success && payload.exists === true) {
                                    backendLog('Phantom already present for this app; not inserting button');
                                    window.__PhantomPresenceCheckInFlight = false;
                                    return; // do not insert
                                }
                                // Re-check in case another caller inserted during async
                                if (!document.querySelector('.phantom-button') && !window.__PhantomButtonInserted) {
                                    // Insert after icon button (order: Restart → Icon → Add)
                                    const iconExisting = buttonHost.querySelector('.phantom-icon-button');
                                    const restartExisting = buttonHost.querySelector('.phantom-restart-button');
                                    if (iconExisting && iconExisting.before) {
                                        iconExisting.before(phantomButton);
                                    } else if (restartExisting && restartExisting.after) {
                                        restartExisting.after(phantomButton);
                                    } else if (referenceBtn && referenceBtn.after) {
                                        referenceBtn.after(phantomButton);
                                    } else {
                                        buttonHost.appendChild(phantomButton);
                                    }
                                    window.__PhantomButtonInserted = true;
                                    backendLog('Phantom button inserted');
                                }
                                window.__PhantomPresenceCheckInFlight = false;
                            } catch (_) {
                                if (!document.querySelector('.phantom-button') && !window.__PhantomButtonInserted) {
            ...
            try {
                ...
                if (!isNaN(appid)) {
                    const pillBtn = buttonHost.querySelector('.phantom-button');
                    if (pillBtn) {
                        // Skip if pills already built for this appid
                        var existingPills = pillBtn.querySelector('.phantom-pills-container');
                        if (!(existingPills && existingPills.dataset.appid === String(appid) && existingPills.dataset.content)) {
                            fetchGamesDatabase().then(function (db) {
                                const btn = buttonHost.querySelector('.phantom-button');
                                if (!btn) return;

                                let pillsContainer = btn.querySelector('.phantom-pills-container');

                                if (!pillsContainer) {
                                    pillsContainer = document.createElement('div');
                                    pillsContainer.className = 'phantom-pills-container';
                                ...
                                fetchFixes(appid).then(function (fixesData) {
                                    ...
                                    if (btn) {
                                        ...
                                        if (span && span.textContent === 'Unplayable') {
                                            span.textContent = lt('Add via Phantom');
                                        }
                                    }

                                    if (showDenuvoPill) {
                                        const pill = document.createElement('span');
                                        pill.className = 'phantom-pill orange';
            ...
            if (!logState.missingOnce) {
                backendLog('Phantom: store button container not found on this page');
    ...
    function onFrontendReady() {
            ...
            fetchSettingsConfig(true).then(function (cfg) {
                ...
                try {
                    if (window.location.hostname === 'store.steampowered.com') {
                        if (localStorage.getItem('phantom millennium disclaimer accepted') !== '1') {
        ...
        setTimeout(function () {
            if (window.GamepadNav && window.GamepadNav.isConnected && window.GamepadNav.isConnected()) {
                backendLog('[Phantom] Gamepad detected - Navigation enabled');

                // Only show visual hint in Big Picture mode
                if (window.__LUATOOLS_IS_BIG_PICTURE__) {
                    const hint = document.createElement('div');
                    hint.id = 'phantom-gamepad-hint';
                    ...
                    if (!document.querySelector('#phantom-gamepad-hint-styles')) {
                        const style = document.createElement('style');
                        style.id = 'phantom-gamepad-hint-styles';
                        style.textContent = '\
                            @keyframes fadeInOut {\
        ...
        try {
            if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                Millennium.callServerMethod('phantom', 'GetInitApisMessage', {
                    contentScriptQuery: ''
                }).then(function (res) {
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        if (payload && payload.message) {
                            ...
                            if (isUpdateMsg) {
                                ...
                                showPhantomConfirm('Phantom', msg, function () {
                ...
                try {
                    if (window.location.hostname === 'store.steampowered.com') {
                        if (!sessionStorage.getItem('PhantomLoadedAppsGate')) {
                            sessionStorage.setItem('PhantomLoadedAppsGate', '1');
                            Millennium.callServerMethod('phantom', 'ReadLoadedApps', {
                                contentScriptQuery: ''
                            }).then(function (res) {
                                try {
                                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
    ...
    document.addEventListener('click', function (evt) {
        ...
        const anchor = evt.target.closest('.phantom-button');
        if (anchor) {
            evt.preventDefault();
            evt.stopPropagation(); // Stop propagation to avoid conflicts
            backendLog('Phantom delegated click');
            try {
                const match = window.location.href.match(/https:\/\/store\.steampowered\.com\/app\/(\d+)/) || window.location.href.match(/https:\/\/steamcommunity\.com\/app\/(\d+)/);
                const appid = match ? parseInt(match[1], 10) : NaN;
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    if (runState.inProgress && runState.appid === appid) {
                        backendLog('Phantom: operation already in progress for this appid');
                        return;
                    }

                    // Helper that continues with the normal add flow
                    const continueWithAdd = function () {
                        if (!document.querySelector('.phantom-overlay')) {
                            showTestPopup();
                        }
                        runState.inProgress = true;
                        runState.appid = appid;
                        Millennium.callServerMethod('phantom', 'StartAddViaPhantom', {
                            appid,
                            contentScriptQuery: ''
                        });
                        startPolling(appid);
                    ...
                    fetch('https://store.steampowered.com/api/appdetails?appids=' + appid + '&filters=basic')
                            ...
                            return fetchGamesDatabase().then(function (db) {
                                try {
                                    ...
                                    if (gameData && gameData.playable === 0) {
                                        // warning modal
                                        showPhantomPlayableWarning('This game may not work, support for it wont be given in our discord', function () {
                                            continueWithAdd();
                                        }, function () { });
                                    } else {
                                        continueWithAdd();
                        ...
                        .catch(function (err) {
                            backendLog('Phantom: DLC check failed: ' + err);
                            continueWithAdd();
                        });
                }
            } catch (_) { }
    ...
    function startPolling(appid) {
        ...
        const timer = setInterval(() => {
            ...
            try {
                Millennium.callServerMethod('phantom', 'GetAddViaPhantomStatus', {
                    appid,
                    contentScriptQuery: ''
                }).then(function (res) {
                    try {
                        ...
                        const overlay = document.querySelector('.phantom-overlay');
                        const title = overlay ? overlay.querySelector('.phantom-title') : null;
                        const status = overlay ? overlay.querySelector('.phantom-status') : null;
                        const wrap = overlay ? overlay.querySelector('.phantom-progress-wrap') : null;
                        const progressInfo = overlay ? overlay.querySelector('.phantom-progress-info') : null;
                        const percent = overlay ? overlay.querySelector('.phantom-percent') : null;
                        const downloadSize = overlay ? overlay.querySelector('.phantom-download-size') : null;
                        const bar = overlay ? overlay.querySelector('.phantom-progress-bar') : null;

                        // Update individual API status in the list
                        if (overlay) {
                            const colors = getThemeColors();
                            const apiItems = overlay.querySelectorAll('.phantom-api-item');
                            ...
                            if ((st.status === 'downloading' || st.status === 'processing' || st.status === 'installing' || st.status === 'done') && st.currentApi && !successfulApi) {
                                ...
                                apiItems.forEach((item) => {
                                    ...
                                    const apiStatus = item.querySelector('.phantom-api-status');
                                    if (!apiStatus) return;
                        ...
                        if (st.status === 'checking' && st.currentApi && title) {
                            title.textContent = lt('Phantom · {api}').replace('{api}', st.currentApi);
                        ...
                        if (st.status === 'downloading' || st.status === 'processing' || st.status === 'installing') {
                            ...
                            const cancelBtn = overlay ? overlay.querySelector('.phantom-cancel-btn') : null;
                            if (cancelBtn && st.status === 'downloading') cancelBtn.style.display = '';
                        }
                        if (st.status === 'done') {
                            // Update popup if visible
                            if (title) title.textContent = t('common.appName', 'Phantom');
                            ...
                            const cancelBtn = overlay ? overlay.querySelector('.phantom-cancel-btn') : null;
                            if (cancelBtn) cancelBtn.style.display = 'none';
                            const hideBtn = overlay ? overlay.querySelector('.phantom-hide-btn') : null;
                            if (hideBtn) hideBtn.innerHTML = '<span>' + lt('Close') + '</span>';
                            ...
                            const btnEl = document.querySelector('.phantom-button');
    ...
    setTimeout(addPhantomButton, 1000);
    setTimeout(addPhantomButton, 3000);
                        ...
                        for (let j = 0; j < nodesToCheck; j++) {
                            ...
                            if (node.nodeType === 1) { // Element node
                                ...
                                if (node.classList && (
                                    ...
                                    node.classList.contains('phantom-store-button-row') ||
    ...
    function showLoadedAppsPopup(apps) {
        // Avoid duplicates
        if (document.querySelector('.phantom-loadedapps-overlay')) return;
        ensureFontAwesome();
        ensurePhantomStyles();
        const overlay = document.createElement('div');
        ...
        overlay.className = 'phantom-loadedapps-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.className = 'phantom-loadedapps-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        const modal = document.createElement('div');
        ...
        title.textContent = lt('Phantom · Added Games');
        const body = document.createElement('div');
        ...
        dismissBtn.className = 'phantom-btn';
        dismissBtn.innerHTML = '<span>' + lt('Dismiss') + '</span>';
        dismissBtn.href = '#';
        dismissBtn.onclick = function (e) {
            e.preventDefault();
            try {
                Millennium.callServerMethod('phantom', 'DismissLoadedApps', {
                    contentScriptQuery: ''
                });
            } catch (_) { }
            try {
                sessionStorage.setItem('PhantomLoadedAppsShown', '1');
            } catch (_) { }
            overlay.remove();
        };
