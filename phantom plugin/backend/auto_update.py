"""Auto-update utilities for the Phantom backend."""
...
def apply_pending_update_if_any() -> str:
    ...
    try:
        ...
        if new_version:
            return f"Phantom updated to {new_version}. Please restart Steam."
        return "Phantom update applied. Please restart Steam."
...
def check_for_update_once() -> str:
    ...
    try:
        ...
        return f"Phantom updated to {latest_version}. Please restart Steam."
...
def _check_and_donate_keys() -> None:
    ...
    try:
        ...
        if not steam_path:
            logger.warn("Phantom: Cannot donate keys - Steam path not found")
            return
        ...
        if pairs:
            ...
            logger.log("Phantom: No valid keys found to donate")
            
    except Exception as exc:
        logger.warn(f"Phantom: Donate keys check failed: {exc}")
...
def restart_steam_internal() -> bool:
    ...
    if not os.path.exists(script_path):
        logger.error(f"Phantom: restart script not found: {script_path}")
        return False
    try:
        CREATE_NO_WINDOW = 0x08000000
        subprocess.Popen(["cmd", "/C", script_path], creationflags=CREATE_NO_WINDOW)
        logger.log("Phantom: Restart script launched (hidden)")
        return True
    except Exception as exc:
        logger.error(f"Phantom: Failed to launch restart script: {exc}")
        return False
...
def check_for_updates_now() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"Phantom: CheckForUpdatesNow failed: {exc}")
